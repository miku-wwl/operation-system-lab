...
changestate(int from, int to, struct proc *p)
{
  ...
  if(from == SLEEPING){
    ...
    // cprintf("pid = %d, start = %d, end = %d, SLEEPING time + %d\n", curproc->pid, curproc->slstart, curproc->slend, curproc->slend - curproc->slstart);
    ...
    // cprintf("pid = %d, start = %d, end = %d, RUNNABLE time + %d\n", curproc->pid, curproc->restart, curproc->reend, curproc->reend - curproc->restart);
    curproc->start = curproc->end;
    if(to == SLEEPING){
    ...
    // cprintf("pid = %d, start = %d, end = %d, RUNNING time + %d\n", curproc->pid, curproc->rustart, curproc->ruend, curproc->ruend - curproc->rustart);
    curproc->start = curproc->end;
  }
  else{
    curproc->start = ticks;
...
userinit(void)
{
  ...
  acquire(&ptable.lock);
  changestate(p->state, RUNNABLE, p);
  p->state = RUNNABLE;
...
fork(void)
{
  ...
  *np->tf = *curproc->tf;
  ...
  acquire(&ptable.lock);
  changestate(np->state, RUNNABLE, np);
  np->state = RUNNABLE;
...
exit(void)
{
  ...
  changestate(curproc->state, ZOMBIE, curproc);
...
yield(void)
{
  ...
  changestate(myproc()->state, RUNNABLE, myproc());
...
sleep(void *chan, struct spinlock *lk)
{
  ...
  // Go to sleep.
  changestate(myproc()->state, SLEEPING, myproc());
  p->chan = chan;
  p->state = SLEEPING;
...
wakeup1(void *chan)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    if(p->state == SLEEPING && p->chan == chan){
      changestate(SLEEPING, RUNNABLE, p);
      p->state = RUNNABLE;
    }
}
